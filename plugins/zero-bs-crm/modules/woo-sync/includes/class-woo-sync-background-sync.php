...
class Woo_Sync_Background_Sync {
	...
	public function sync_orders(){
		...
		if ( $is_external_wc ) {
			...
			if ( empty( $domain ) || empty( $key ) || empty( $secret ) ) {
				...
				return array(
					...
					'status_long_text'  => __( 'WooSync will start importing data when you have updated your settings.', 'zero-bs-crm' ),
		...
		return array_merge( $woosync_hub_info, $this->woosync()->get_jpcrm_woo_latest_stats() );
	...
	public function import_orders_from_store( $page_no = -1 ) {
		...
		if ( $orders->max_num_pages == 0 ) {
			...
			if ( $this->is_cron() ) {
				...
				echo json_encode( array(
					...
					'status_short_text'    => $status_short_text,
					'status_long_text'     => __( 'You currently have no WooCommerce orders. As orders get added, they will be automatically imported!', 'zero-bs-crm' ),
					'page_no'              => $page_no,
					'orders_imported'      => 0,
		...
		if ( $page_no >= $orders->max_num_pages ) {
			...
			if ( $this->is_cron() ) {
				...
				echo json_encode( array_merge( $woosync_hub_info, $this->woosync()->get_jpcrm_woo_latest_stats() ) );
				exit();
	...
	public function import_orders_from_api( $page_no = -1 ) {
		...
		try {
			...
			if ( $total_pages === 0 ) {
				...
				if ( $this->is_cron() ) {
					...
					echo json_encode( array(
						'status'               => 'sync_completed',
						'status_short_text'    => $status_short_text,
						'status_long_text'     => __( 'There were no WooCommerce orders to import from this API endpoint. As orders get added, they will be automatically imported!', 'zero-bs-crm' ),
						'page_no'              => $page_no,
						'orders_imported'      => 0,
						'percentage_completed' => 100,
					));
			...
			foreach ( $orders as $order ) {
				...
				$this->debug( 'Importing order: ' . $order->number . ' (becoming: ' . $this->woosync()->get_prefix() . $order->number . ')' );

				// prefix ID and number
				$order->number = $this->woosync()->get_prefix() . $order->number;
			...
			if ( $page_no >= $total_pages ) {
				...
				// set pointer to last page
				$this->set_resume_from_page( $total_pages );
	...
	public function resume_from_page(){
	    ...
	    return (int)get_option( 'zbs_woo_resume_sync', 0 );
	...
	public function percentage_completed() {
		...
		if ( $this->import_mode() == JPCRM_WOO_SYNC_MODE_API ) {
			...
			try {
				...
				$total_pages         = $lc_response_headers['x-wp-totalpages'];
			...
			if ( !function_exists( 'wc_get_orders' ) ) {
				...
				if ( $page_no > 0 && $orders->max_num_pages > 0 ) {
						...
						$percentage_completed = $page_no / ($orders->total/$this->orders_per_page) * 100;
