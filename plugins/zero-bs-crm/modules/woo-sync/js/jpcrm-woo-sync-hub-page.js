		...
		*/
		...
		if ( typeof response.status != "undefined" ){
			...
			if ( response.last_order_synced ) {
				document.getElementById('jpcrm-woosync-stat-last-order-synced').innerHTML = response.last_order_synced;
			}
			if ( response.contacts_synced ) {
				document.getElementById('jpcrm-woosync-stat-contacts-synced').innerHTML = response.contacts_synced;
			}
			if ( response.transactions_synced ) {
				document.getElementById('jpcrm-woosync-stat-transactions-synced').innerHTML = response.transactions_synced;
			}
			if ( response.transaction_total ) {
				document.getElementById('jpcrm-woosync-stat-transaction-total').innerHTML = response.transaction_total;
			}
			...
			switch ( response.status ){
				...
				case 'sync_part_complete':
					...
					document.getElementById('jpcrm-woosync-status-short-text').textContent = response.status_short_text + ' (' + Math.round(response.percentage_completed) + '%)';
				...
				case 'sync_completed':
					...
					document.getElementById('jpcrm-woosync-status-icon').className = 'icon thumbs up green';
					document.getElementById('jpcrm-woosync-status-short-text').textContent = response.status_short_text;
					document.getElementById('jpcrm-woosync-status-long-text').textContent = response.status_long_text;
					return;
				...
				case 'error':
					...
					document.getElementById('jpcrm-woosync-status-icon').className = 'icon settings orange';
					document.getElementById('jpcrm-woosync-status-short-text').textContent = response.status_short_text;
					document.getElementById('jpcrm-woosync-status-short-text').className = 'status orange';
					document.getElementById('jpcrm-woosync-status-long-text').textContent = response.status_long_text;
					return;
			...
			if ( typeof window.jpcrm_woosync_sync_blocked_runs == "undefined" || window.jpcrm_woosync_sync_blocked_runs < 10 ){
					...
					error_string = jpcrm_woosync_language_label( error_type, 'Import job is running in the back end. If this message is still shown after some time, please contact support.' );
			...
			if ( response.statusText == 'timeout' ) {
					...
					error_string = jpcrm_woosync_language_label( error_type, 'There was a general server error.' ) + ' (' + response.status + ')';
