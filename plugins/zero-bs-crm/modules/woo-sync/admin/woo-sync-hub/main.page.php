...
function jpcrm_woosync_render_hub_page() {
	...
	$is_external_wc = ( $settings['wcsetuptype'] == 1 );
	...
	if ( $is_external_wc ) {
		...
		if ( empty( $domain ) || empty( $key ) || empty( $secret ) ) {
			...
			$error_messages[] = zeroBSCRM_UI2_messageHTML( 'warning', '', sprintf( __( 'You have not setup your WooCommerce API details. Please fill in your API details on the <a href="%s" target="_blank">settings page</a>.', 'zero-bs-crm' ), $settings_page_url ) );

			// if no prefix, alert
			// really we could move this out of this block, but it would
			// get messy if they've already started an import
			if ( empty( $prefix ) ) {
				$error_messages[] = zeroBSCRM_UI2_messageHTML( 'info', '', sprintf( __( 'You are set up to import from an external site, but you have not set an order prefix. This is recommended so that orders from external sites do not clash with local/other site orders. Please add a prefix on the <a href="%s" target="_blank">settings page</a>.', 'zero-bs-crm' ), $settings_page_url ) );
			}
		...
		if ( !$zbs->woocommerce_is_active() ) {
			...
			$error_messages[] = zeroBSCRM_UI2_messageHTML( 'warning', '', __( 'You do not have WooCommerce installed. Please first install WooCommerce to use this function.', 'zero-bs-crm' ) );
	...
	$has_woosync_errors = count( $error_messages ) > 0;
	...
	<div id="jpcrm-woosync-hub-page">
		...
		if ( isset( $sync_restarted ) ) {
			...
			<div id="jpcrm-woosync-messages">
				<?php echo zeroBSCRM_UI2_messageHTML( 'info', __( 'Sync restarted', 'zero-bs-crm' ), __( 'The WooSync import has been restarted. This will start running in the background from the beginning.', 'zero-bs-crm' ) ); ?>
			</div>
			<?php
		}
		?>
		<div class="ui segment" id="jpcrm-woosync-page-body">
			...
			<h2 class="ui header">
				...
				<div class="content">
					...
					<span id="jpcrm-woosync-status-short-text" class="status green"><?php _e( 'Syncing content from WooCommerce...', 'zero-bs-crm' ); ?></span>
					...
					<div class="sub header">
						...
						<br>
						<span id="jpcrm-woosync-status-long-text"><?php _e( 'WooSync is importing orders...', 'zero-bs-crm' ); ?></span>
